...
class plgCCK_FieldLink extends JCckPluginField
{
	...
	public function onCCK_FieldPrepareContent( &$field, $value = '', &$config = array() )
	{
		...
		if ( $value != '' ) {
			...
			$field->text		=	$text;
			$field->link  		=	( strpos( $link, 'index.php' ) === 0 || $link[0] == '/' ) ? $link : ( ( strpos( $link, 'http://' ) === false ) ? 'http://'.$link : $link );
	...
	{		
		...
		if ( $field->bool5 == 1 && $config['pk'] && @$value['link'] != '' ) {
			...
			$p_link		=	( strpos( $p_link, 'index.php' ) === 0 ) ? $p_link : ( ( strpos( $p_link, 'http://' ) === false ) ? 'http://'.$p_link : $p_link );
