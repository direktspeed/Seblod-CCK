...
class plgSystemCCK extends JPlugin
{
	...
	function plgSystemCCK( &$subject, $config )
	{
		...
		if ( $app->isAdmin() ) {
			JFactory::getLanguage()->load( 'lib_cck', JPATH_ADMINISTRATOR );
	...
	public function onAfterInitialise()
	{
		...
		if ( $user->get( 'id' ) > 0 && @$user->guest != 1 ) {
			...
			$nolevels	=	explode( ',', implode( ',', $nolevels ) );
	...
	public function onAfterDispatch()
	{
		...
		if ( $app->isAdmin() ) {			
			switch ( $option ) {
				case 'com_cck':
					if ( $view == 'types' || $view == 'fields' || $view == 'searchs' || $view == 'templates' ) {
						if ( JCck::getUser_Value( 'preferences_'.$view.'_splash_screen', '1' ) == 2 ) {
							JCckDevIntegration::addDropdown( $view );
						}
					} elseif ( $view == 'sites' ) {
						if ( JCck::getUser_Value( 'preferences_'.$view.'_splash_screen', '2' ) == 2 ) {
							JCckDevIntegration::addDropdown( $view );
						}
					}
					break;
				...
				case 'com_modules':
					if ( $layout ) {
						...
						if ( (!$view || $view == 'modules') && !$app->getUserState( 'com_modules.modules.filter.client_id', 0 ) ) {
							if ( JCck::getUser_Value( 'preferences_modules_splash_screen', '2' ) == 2 ) {
								JCckDevIntegration::addDropdown( 'module' );
							}
						}
					}
					break;
			...
			if ( !JCck::getConfig_Param( 'sef_canonical', 0 ) && !isset( $app->cck_canonical ) && JCck::on() && count( $doc->_links ) ) {
	...
	protected function _setBasics( $buffer, $option, $view )
	{
		...
		if ( !JCck::on() ) {
			if ( $option == 'com_cck' ) {
				if ( $view == 'types' ) {
					$replace	=	'<a href="index.php?option=com_cck&view=versions&e_type=type&e_id=0">'.JText::_( 'COM_CCK_VERSIONS' ).'</a>';
					$buffer		=	$this->_reSubmenu( $buffer, '#<ul id="submenu">(.*)</ul>#sU', $replace );
				} elseif( $view == 'searchs' ) {
					$replace	=	'<a href="index.php?option=com_cck&view=versions&e_type=search&e_id=0">'.JText::_( 'COM_CCK_VERSIONS' ).'</a>';
					$buffer		=	$this->_reSubmenu( $buffer, '#<ul id="submenu">(.*)</ul>#sU', $replace );
