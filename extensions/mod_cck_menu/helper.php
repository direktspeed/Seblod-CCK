...
class modCckMenuHelper
{
	...
	public static function buildMenu( $mode, $menutitle, $moduleid, $com, $more = array() )
	{
		...
		if ( $mode == 1 || $mode == 2 ) {
			...
			$menu->addChild( new JCCKMenuNode( '- Liked', 'http://www.seblod.com/products/liked', '', false, '_blank' ) );
			...
			$menu->addChild( new JCCKMenuNode( '- Live Support', 'http://www.seblod.com/services/live-support', '', false, '_blank' ) );
			$menu->addChild( new JCCKMenuNode( '- Ticket Support', 'http://www.seblod.com/services/ticket-support', '', false, '_blank' ) );
