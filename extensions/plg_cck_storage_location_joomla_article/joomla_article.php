...
class plgCCK_Storage_LocationJoomla_Article extends JCckPluginLocation
{
	...
	protected static $to_route	=	'a.id as pk, a.title, a.alias, a.catid';
	...
	protected function _completeTable( &$table, &$data, &$config )
	{
		...
		$intro	=	JCckDatabase::loadResult( $query );
		if ( $intro > 0 ) {
			if ( isset( $config['params'] ) ) {
	...
	public static function getRoute( $item, $sef, $itemId, $config = array() )
	{
		$route		=	'';
		if ( is_numeric( $item ) ) {
			$item	=	JCckDatabaseCache::loadObject( 'SELECT a.id, a.alias, a.catid, b.alias AS category_alias'
		...
		if ( $sef ) {
			...
			require_once JPATH_SITE.'/components/com_content/helpers/route.php';
			$route		=	ContentHelperRoute::getArticleRoute( $item->slug, $item->catid );
		}
	...
	public static function getStaticProperties( $properties )
	{
		if ( count( $properties ) ) {
			foreach ( $properties as $i=>$p ) {
				if ( $p == 'key' || $p == 'table' || $p == 'access' || $p == 'custom' || $p == 'status' || $p == 'to_route' || $p == 'contexts' || $p == 'context' ) { // todo: clean!
					$properties[$p]	=	self::${$p};
				}
				unset( $properties[$i] );
			}
