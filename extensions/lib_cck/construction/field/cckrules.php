...
if ( JCck::on() ) {
	...
	class JFormFieldCCKRules extends JFormField
	{
		...
		protected function getInput()
		{
			...
			foreach ($groups as $group)
			{
				...
				foreach ($actions as $action)
				{
					...
					if ( $action->input_type == 'text' ) {
						...
						$html[]	=	'<input class="inputbox" type="text" name="' . $this->name . '[' . $action->name . '][' . $group->value . ']" id="' . $this->id . '_' . $action->name . '_' . $group->value . '" title="' . JText::sprintf('JLIB_RULES_SELECT_ALLOW_DENY_GROUP', JText::_($action->title), trim($group->text)) . '" value="'.$value.'" size="8" />';
					} else {
					
						$html[] = '<select class="inputbox" name="' . $this->name . '[' . $action->name . '][' . $group->value . ']" id="' . $this->id . '_' . $action->name . '_' . $group->value . '" title="' . JText::sprintf('JLIB_RULES_SELECT_ALLOW_DENY_GROUP', JText::_($action->title), trim($group->text)) . '">';
					...
					if ($canCalculateSettings) {
						...
						if (CCKAccess::checkGroup($group->value, 'core.admin', $assetId) !== true)
						{
							...
							elseif ($inheritedRule === true)
							{
								$html[] =	'<span class="icon-16-allowed">' . $allowed . '</span>';
							}
							elseif ($inheritedRule === false) {
								if ($assetRule === false) {
			...
			$js = "window.addEvent('domready', function(){ new Fx.Accordion($$('div#permissions-sliders.pane-sliders .panel h3.pane-toggler'), $$('div#permissions-sliders.pane-sliders .panel div.pane-slider'), {onActive: function(toggler, i) {toggler.addClass('pane-toggler-down');toggler.removeClass('pane-toggler');i.addClass('pane-down');i.removeClass('pane-hide');Cookie.write('jpanesliders_permissions-sliders".$component."',$$('div#permissions-sliders.pane-sliders .panel h3').indexOf(toggler));},onBackground: function(toggler, i) {toggler.addClass('pane-toggler');toggler.removeClass('pane-toggler-down');i.addClass('pane-hide');i.removeClass('pane-down');},duration: 300,display: ".$input->cookie->getInt('jpanesliders_permissions-sliders'.$component, 0).",show: ".$input->cookie->getInt('jpanesliders_permissions-sliders'.$component, 0).", alwaysHide:true, opacity: false}); });";
