...
class CCK_Rendering
{
	...
	{	
		...
		$css_override	=	$this->path.'/css/'.$this->client.'.css';
	...
	public function setConditionalStates( &$field )
	{
		...
		$this->addJS( '$("#'.$this->id.'_'.$field->name.'").conditionalStates('.$field->conditional_options.');' );
	}
	...
	public function renderPosition( $position = '', $variation = '', $height = '', $excluded = array(), $force = false )
	{
        $html	=	'';
		if ( ! $variation ) {
			$variation	=	( @$this->positions_m[$position]->variation ) ? $this->positions_m[$position]->variation : $this->params['variation_default'];
		}
		
		if ( @$this->positions_m[$position]->variation_options != '' ) {
			$options	=	new JRegistry;
		...
		$legend		=	( @$this->positions_m[$position]->legend ) ? trim( $this->positions_m[$position]->legend ) : (( $this->doDebug() ) ? $position : '' );
