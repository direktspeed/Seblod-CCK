...
$preconfig['show_form']		=	( $preconfig['show_form'] != '' ) ? $preconfig['show_form'] : $options->get( 'show_form', 1 );
...
if ( $preconfig['show_form'] ) {
	...
	$templateStyle	=	CCK_List::getTemplateStyle( $search->$P );
	if ( ! $templateStyle ) {
...
foreach ( $fields as $field ) {
	...
	if ( $field->variation_override ) {
		$override	=	JCckDev::fromJSON( $field->variation_override );
	...
	// Value
	if ( ( !$field->variation || $field->variation == 'form_filter' ) && isset( $post[$name] ) ) {
		$value	=	$post[$name];
	} else {
		if ( isset( $lives[$name] ) ) {
			$value		=	$lives[$name];
	...
	if ( $preconfig['show_list'] ) {
		if ( $total ) {
			...
			if ( $options->get( 'cache2' ) ) {
				...
				if ( $search->content > 0 ) {
					$data	=	CCK_List::render( $items, $search, $path, $preconfig['client'], $preconfig['itemId'], $options );
				}
				$isCached	=	' [Cache=OFF]';
			}
		} else {
			...
			if ( $no_action ) {
				if ( $no_action == 'auto_redirect' ) {
					...
					$templateStyle	=	CCK_List::getTemplateStyle( $search->template_list );
