...
for ( $i = 0; $i < $count; $i++ ) {
	...
	if ( $count2 ) {
		$config		=	array( 'author'=>0,
							   ...
							   'storages'=>array(),
							   'type'=>$list[$i]->cck,
							   'type_id'=>(int)$list[$i]->type_id,
							   'type_alias'=>( $list[$i]->type_alias ? $list[$i]->type_alias : $list[$i]->cck )
							);
		$fieldsI	=	array();
	...
	$item->init();
	$doc->list[$PK]				=	$item;
	$doc->list[$PK]->pid		=	$list[$i]->pid;
	$doc->list[$PK]->pk			=	$list[$i]->pk;
	$doc->list[$PK]->pkb		=	$list[$i]->pkb;
	$doc->list[$PK]->cck		=	$list[$i]->cck;
	$doc->list[$PK]->location	=	$list[$i]->loc;
}
?>