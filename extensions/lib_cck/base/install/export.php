...
class CCK_Export
{
	...
	public static function findFields( $paths, $dest = '' )
	{
		if ( count( $paths ) ) {
			foreach ( $paths as $path ) {
				...
				if ( $names != '' ) {
					$fields	=	JCckDatabase::loadObjectList( 'SELECT a.* FROM #__cck_core_fields AS a WHERE a.name IN ('.$names.') AND a.id >= 500' );
					if ( $dest ) {
	...
	public static function exportType( $elemtype, $elem, &$xml, &$data, &$extensions, &$file )
	{
		...
		foreach ( $elems as $el ) {
			...
			self::exportElement( 'field', $fields[$el->fieldid], $data, $extensions, 500 );
			$i++;
		}
	...
	public static function exportFile( $type, &$data, $file )
	{
		...
		if ( $file['src'] && JFolder::exists( $file['src'] ) ) {
			if ( $type == 'variation' ) {
				...
				if ( $type == 'plugin' ) {
					CCK_Export::findFields( array( $file['src'].'/tmpl/edit.php' ), $path.'/install' );
				}
			}
			if ( @$file['lang_src'] != '' ) {
