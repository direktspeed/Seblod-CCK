...
{
	// importContent
	public static function importContent( $type, $base, $items, &$data )
	{
					...
					if ( isset( $tables[$short] ) ) {
						...
						if ( count( $fields ) ) {
							foreach ( $fields as $field ) {
								...
								if ( !isset( $table_fields[$column] ) ) {
									$query	=	'ALTER TABLE '.$name.' ADD '.$column.' '.$type.' NOT NULL';
									$query	.=	( $default != '' ) ? ' DEFAULT "'.$default.'"' : '';
									$query	.=	( $previous != '' ) ? ' AFTER '.$previous : ' FIRST';
									JCckDatabase::execute( $query );
								} else {
									if ( $type != $table_fields[$column]->Type ) {
										$query	=	'ALTER TABLE '.$name.' CHANGE '.$column.' '.$column.' '.$type.' NOT NULL';
										$query	.=	( $default != '' ) ? ' DEFAULT "'.$default.'"' : '';
										JCckDatabase::execute( $query );
									}
								}
						...
						if ( count( $fields ) ) {
							foreach ( $fields as $field ) {
								$type		=	(string)$field->attributes()->type;
								$default	=	(string)$field->attributes()->default;
								$sql_query	.=	' '.(string)$field.' '.$type.' NOT NULL';
								if ( $default != '' ) {
