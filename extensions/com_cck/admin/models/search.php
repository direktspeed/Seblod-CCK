...
class CCKModelSearch extends JCckBaseLegacyModelAdmin
{
	...
	protected function populateState()
	{
		...
		if ( ! ( $pk = $app->input->getInt( 'id', 0 ) ) ) {
			if ( $tplScreen	=	(int)$app->getUserState( CCK_COM.'.add.search.splash_screen' ) ) {
				jimport( 'cck.content.user' );
				CCK_User::setPreference( 'searchs_splash_screen', 0 );
			} else {
				jimport( 'cck.content.user' );
				if ( JCck::getUser_Value( 'preferences_searchs_splash_screen', 1 ) == 0 ) {
					CCK_User::setPreference( 'searchs_splash_screen', 2 );
				}
			}
	...
	{
		foreach ( $pks as $pk ) {
			Helper_Version::createVersion( 'search', $pk );
		}
