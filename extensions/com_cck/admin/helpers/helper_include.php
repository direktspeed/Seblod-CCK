...
{	
	...
	public static function addDependencies( $view, $layout, $tmpl = '' )
	{
		...
		switch ( $view ) {
			case 'box':
				JCck::loadjQuery( true, true, array( 'cck.dev-3.0.0.min.js', 'jquery.ui.effects.min.js', 'jquery.json.min.js' ) );
			...
			case 'field':
				if ( $script === true ) {
					JCck::loadjQuery( true, true, array( 'cck.dev-3.0.0.min.js' ) );
					JCck::loadjQueryUI();
				}
			...
			case 'search':
				if ( $script === true ) {
					JCck::loadjQuery( true, true, array( 'cck.dev-3.0.0.min.js', 'jquery.biscuit.min.js' ) );
					JCck::loadjQueryUI();
					$doc->addStyleSheet( JROOT_CCK.'/administrator/components/com_'.CCK_NAME.'/assets/css/ui-construction.css' );
					$doc->addStyleSheet( JROOT_CCK.'/administrator/components/com_'.CCK_NAME.'/assets/styles/'.
											JCck::getUser_Value( 'preferences_style', 'indigo_dye' ).'/ui-construction.css' );
				}
