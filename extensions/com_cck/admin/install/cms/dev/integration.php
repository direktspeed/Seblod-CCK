...
abstract class JCckDevIntegration
{
	...
	public static function addDropdown( $view, $variables = '', $options = NULL )
	{
		...
		if ( count( $items ) && $html != '' ) {			
			if ( JCck::on() ) {
				...
				$css	=	'.subhead .dropdown-menu {text-shadow: none;}';
				$js		=	'
	...
	{
		$doc	=	JFactory::getDocument();
		$desc	=	htmlspecialchars( JText::_( 'LIB_CCK_INTEGRATION_WARNING_COPY' ), ENT_COMPAT, 'UTF-8' );
		$html	=	'<img id="cck_warning" class="hasTip2" title="'.$desc.'"'
	...
	public static function rewriteBuffer( $buffer, $data, $list, $list_assoc = array() )
	{
		...
		if ( JCck::on() ) {
			if ( $data['search_alt'] ) {
				...
				if ( count( $matches2[3] ) ) {
					if ( $multilanguage ) {
						// $pks		=	( $pks ) ? substr( $pks, 0, -1 ) : implode( ',', $matches2[2] ); 
					...
					foreach ( $matches2[3] as $k=>$m ) {
						...
						if ( isset( $matches[0][$k] ) ) {
							...
							if ( $opt_edit_alt ) {
								if ( isset( $list2[$pk] ) ) {
									$replace	=	$m.' ('.JText::_( 'LIB_CCK_LEGACY' ).') '.'</a>';
									if ( $multilanguage ) {
										...
										if ( $list[$pk]->key ) {
											...
											if ( $t_edit ) {
												$replace	.=	'<li><a href="#">'.JText::_( 'LIB_CCK_TRANSLATE_EDIT' ).'</a></li>'.$t_edit;
											}
											if ( $t_add ) {
												$replace	.=	'<li><a href="#">'.JText::_( 'LIB_CCK_TRANSLATE' ).'</a></li>'.$t_add;
											}
										}
									}
									$matches0	=	'<a href="'.$list2[$pk]['link'].'">'.$m.'</a></li><li>'.str_replace( $search, $replace, $matches2[0][$k] );
								} else {
									$link		=	'index.php?option=com_cck&amp;view=form'.$return.'&type='.$opt_default_type.'&id='.$pk.$data['replace_end'];
									$matches0	=	$matches2[0][$k].'</li><li><a href="'.$link.'">'.$m.' ('.JText::_( 'LIB_CCK_SEBLOD' ).') '.'</a>';
								}
								$buffer		=	str_replace( $matches2[0][$k], $matches0, $buffer );
							} else {
								if ( isset( $list2[$pk] ) ) {
									$matches0	=	'<a href="'.$list2[$pk]['link'].'">'.$m.'</a>';
									// todo
									$buffer		=	str_replace( $matches2[0][$k], $matches0, $buffer );
								}
							}
