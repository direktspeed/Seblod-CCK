...
class JCckPluginField extends JPlugin
{
	...
	public static function g_onCCK_FieldConstruct_SearchSearch( &$field, $style, $data )
	{
		...
		// 2
		if ( @$field->live != '' && @$field->live != 'stage' ) {
			$hide0	=	' hide';
			$hide	=	' hide';
			$hide2	=	'';
		} else {
	...
	public static function g_getDisplayVariation( &$field, $variation, $value, $text, $form, $id, $name, $html, $hidden = '', $more = '', $config = array() )
	{
		...
		if ( $variation == 'value' ) {
			$base			=	( $hidden != '' ) ? trim( $hidden ) : '<input type="hidden" id="'.$id.'" name="'.$name.'" value="'.$value.'" class="'.$class.'" />';
			$field->form	=	$base . '<span id="_'.$id.'" class="variation_value">'.$text.'</span>';
		} elseif ( $variation == 'disabled' ) {
			$base			=	( $hidden != '' ) ? trim( $hidden ) : '<input type="hidden" id="_'.$id.'" name="'.$name.'" value="'.$value.'" class="'.$class.'" />';
			$field->form	=	$base;
			if ( $html ) {
			...
			self::g_addScriptDeclaration( '$("form#'.$parent.'").on("change", "#'.$id.'", function() { '.$submit.'(\'form.save\'); });' );
