...
<div class="<?php echo $this->css['items']; ?>">
	...
	foreach ( $this->items as $i=>$item ) {
		...
		$linkVersion	=	JRoute::_( 'index.php?option='.$this->option.'&view=versions&e_type=search&e_id='.$item->id );
		...
		<tr class="row<?php echo $i % 2; ?>" height="64px;">
			...
			<td width="30px" class="center hidden-phone">
            	<a target="_blank" href="<?php echo $link2; ?>"><img src="components/<?php echo CCK_COM; ?>/assets/images/24/icon-24-searchs.png" border="0" alt="" title="<?php echo $title2 ?>" /></a>
			</td>
			<td>
			...
			<td width="3%" class="center hidden-phone">
				<?php echo ( $item->versions ) ? '<a href="'.$linkVersion.'"><img src="components/'.CCK_COM.'/assets/images/16/icon-16-version.png" border="0" alt="" /></a>' : ''; ?>
			</td>
...
(function ($){
	JCck.Dev = {
		...
		addNew: function(skip) {
			...
			var tpl_i = "seb_one";
			var url = "index.php?option=com_cck&task=search.add&splash_screen="+splash+"&content_type="+featured+"&tpl_s="+tpl_s+"&tpl_f="+tpl_f+"&tpl_l="+tpl_l+"&tpl_i="+tpl_i+"&skip="+skip;
	...
	$(document).ready(function() {
		...
		$("#toolbar-new > button").on('click', function () {
			JCck.Dev.status = 1;
		});
		$("#toolbar-new > button").on('mouseleave', function () {
			if (JCck.Dev.status == 1) {
				JCck.Dev.status = 0;
				JCck.Dev.addScroll();
			}
		});
