...
if ( ( JCck::getConfig_Param( 'validation', 2 ) > 1 ) && $this->config['validation'] != '' ) {
	...
	$js	=	'if (jQuery("#'.$this->config['formId'].'").validationEngine("validate",task) === true) { Joomla.submitform("search", document.getElementById("'.$this->config['formId'].'")); }';
} else {
	$js	=	'Joomla.submitform("search", document.getElementById("'.$this->config['formId'].'"));';
}
...
Joomla.submitbutton = function(task, cid)
{
	if (task == "<?php echo $this->vName; ?>.delete") {
		if (confirm(Joomla.JText._('COM_CCK_CONFIRM_DELETE'))) {
			jQuery("#system").after('<input type="hidden" id="return" name="return" value="<?php echo base64_encode( JFactory::getURI() ); ?>">');
