...
<div class="<?php echo $this->css['items']; ?>">
	...
	foreach ( $this->items as $i=>$item ) {
		...
		$linkVersion	=	JRoute::_( 'index.php?option='.$this->option.'&view=versions&e_type=type&e_id='.$item->id );
		...
		<tr class="row<?php echo $i % 2; ?>" height="64px;">
			...
			<td width="30px" class="center hidden-phone">
            	<?php if ( $item->published && $item->adminFields && $item->location != 'site' && $item->storage_location != 'none' && $canCreateItem ) { ?>
					<a target="_self" href="<?php echo $link2; ?>"><img src="components/<?php echo CCK_COM; ?>/assets/images/24/icon-24-forms.png" border="0" alt="" title="<?php echo $title2 ?>" /></a>
                <?php } ?>
			</td>
			<td>
...
(function ($){
	JCck.Dev = {
		addNew: function(id) {
			...
			var tpl_c = "seb_one";
			var tpl_i = "seb_one";
			var url = "index.php?option=com_cck&task=type.add&splash_screen="+splash+"&skeleton_id="+id+"&tpl_a="+tpl_a+"&tpl_s="+tpl_s+"&tpl_c="+tpl_c+"&tpl_i="+tpl_i;
