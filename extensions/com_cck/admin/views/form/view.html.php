...
class CCKViewForm extends JViewLegacy
{
	...
	protected function addToolbar( $title = '', $name = '' )
	{
		...
		$bar->appendButton( 'CckLink', 'apply', 'JTOOLBAR_APPLY', 'javascript:JCck.Core.submit(\'form.apply\');' );
		$bar->appendButton( 'CckLink', 'save', 'JTOOLBAR_SAVE', 'javascript:JCck.Core.submit(\'form.save\');' );
		$bar->appendButton( 'CckLink', 'save-new', 'JTOOLBAR_SAVE_AND_NEW', 'javascript:JCck.Core.submit(\'form.save2new\');' );
		if ( $this->isNew )  {
			...
			$bar->appendButton( 'CckLink', 'cancel', 'JTOOLBAR_CANCEL', 'javascript:JCck.Core.submit(\'form.cancel\');' );
		} else {
			...
			JToolBarHelper::title( $title, 'form.png' );
			$bar->appendButton( 'CckLink', 'cancel', 'JTOOLBAR_CLOSE', 'javascript:JCck.Core.submit(\'form.cancel\');' );
		}
	}
}
?>