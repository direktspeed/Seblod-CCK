...
class plgCCK_Storage_LocationJoomla_User extends JCckPluginLocation
{
	...
	public function onCCK_Storage_LocationPrepareItems( &$field, &$storages, $pks, &$config = array(), $load = false )
	{
		...
		$config['author']	=	$storages[self::$table][$config['pk']]->{self::$key};
	}
	...
	protected static function _sendMails( $table, $activation, $auto_email, $admin_emails )
	{
		...
		if ( ( $activation < 2 ) && ( $admin_emails == 1 ) ) {
			...
			foreach ( $rows as $row ) {
				...
				if ( $return !== true ) {
					$this->setError( JText::_( 'COM_CCK_REGISTRATION_ACTIVATION_NOTIFY_SEND_MAIL_FAILED' ) );
					return false;
				}
			}
		}
	...
	public static function getRoute( $item, $sef, $itemId, $config = array() )
	{
		if ( is_numeric( $item ) ) {
			...
			$pk		=	JCckDatabase::loadResult( 'SELECT pkb FROM #__cck_core WHERE storage_location = "'.self::$type.'" AND pk = '.(int)$item->id );
			if ( !$pk ) {
				return '';
			}
		}
