...
class plgContentCCKInstallerScript
{
	...
	function install( $parent )
	{
		...
		$groups	=	array( 'cck_field', 'cck_field_link', 'cck_field_live', 'cck_field_typo', 'cck_field_validation', 'cck_storage', 'cck_storage_location' );
	...
	{
		$ini	=	'language/en-GB/en-GB.lib_cck.ini';
		if ( JFile::exists( JPATH_ADMINISTRATOR.'/'.$ini ) ) {
			JFile::delete( JPATH_ADMINISTRATOR.'/'.$ini );
		}
		$ini	=	'language/fr-FR/fr-FR.lib_cck.ini';
		if ( JFile::exists( JPATH_ADMINISTRATOR.'/'.$ini ) ) {
			JFile::delete( JPATH_ADMINISTRATOR.'/'.$ini );
		}
	...
	function postflight( $type, $parent )
	{
		...
		if ( $type == 'install' ) {
			...
			if ( count( $pks ) ) {
				...
				$rules2	=	array( 8=>'{"core.create":{"1":1},"core.create.max.parent":{"8":0},"core.create.max.parent.author":{"8":0},"core.create.max.author":{"8":0},'
			...
			$versions	=	array(	0=>'2.0.0', 1=>'2.0.0.RC2', 2=>'2.0.0.RC2-1', 3=>'2.0.0.RC2-2', 4=>'2.0.0.RC2-3', 5=>'2.0.0.RC3', 6=>'2.0.0.RC4',
									...
									54=>'3.2.0', 55=>'3.2.1', 56=>'3.2.2', 57=>'3.5.0' );
			// ******** ******** ******** ******** ******** ******** ******** ******** ******** ******** ******** ******** ******** ******** ******** ******** //
		...
		// WAITING FOR JOOMLA 1.7.x FIX
		$ini	=	'language/en-GB/en-GB.lib_cck.ini';
		if ( JFile::exists( JPATH_SITE.'/'.$ini ) ) {
			JFile::copy( JPATH_SITE.'/'.$ini, JPATH_ADMINISTRATOR.'/'.$ini );
		}
		$ini	=	'language/fr-FR/fr-FR.lib_cck.ini';
		if ( JFile::exists( JPATH_SITE.'/'.$ini ) ) {
			JFile::copy( JPATH_SITE.'/'.$ini, JPATH_ADMINISTRATOR.'/'.$ini );
		...
		if ( JFolder::exists( $path ) ) {
			...
			foreach ( $folders as $folder ) {
				...
				if ( version_compare( JVERSION, $version, 'ge' ) ) {
					$path	.=	'/'.$folder;
					$len	=	strlen( $path );
