...
class plgCCK_Field_ValidationAjax_Availability extends JCckPluginValidation
{
	...
	public static function onCCK_Field_ValidationPrepareForm( &$field, $fieldId, &$config )
	{
		...
		if ( $field->value ) {
			$pk		=	(int)JCckDatabase::loadResult( 'SELECT '.$validation->key.' FROM '.$validation->table.' WHERE '.$validation->column.'="'.$field->value.'"'.$and );
			$extra	.=	'&avKey='.$validation->key.'&avPk='.$pk.'&avPv='.htmlspecialchars( str_replace( "'", '%27', $field->value ) );
		}
		
		$rule		=	'
					"'.$name.'":{
						"url": "index.php?option=com_cck&task=ajax&format=raw&file=/plugins/cck_field_validation/ajax_availability/assets/ajax/script.php",
